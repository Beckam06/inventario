...
function join_product_table() {
    global $db;
    $sql  = "SELECT DISTINCT p.id_producto, p.nombreProducto, p.marca, p.modelo, p.descripcion, ";
    $sql .= "p.cantidad, p.precio, p.proveedor, c.categoria AS categorie, p.fechaIngreso, ";
    $sql .= "p.stock_minimo, cu.cubiculo ";
    $sql .= "FROM producto p ";
    $sql .= "LEFT JOIN categoria c ON c.id_categoria = p.id_categoria ";
    $sql .= "LEFT JOIN cubiculos cu ON cu.id_cubiculo = p.id_cubiculo ";
...
{
  global $db;
  $sql  = "SELECT p.*, c.categoria, cu.cubiculo ";
  $sql .= "FROM producto p ";
  $sql .= "LEFT JOIN categoria c ON c.id_categoria = p.id_categoria ";
  $sql .= "LEFT JOIN categoria_cubiculo cc ON cc.id_categoria = p.id_categoria ";
  $sql .= "LEFT JOIN cubiculos cu ON cu.id_cubiculo = cc.id_cubiculo ";
...
function generar_pdf_orden_salida($id_orden_salida, $id_solicitud, $id_departamento, $responsable, $cantidad_entregada, $nombre_producto, $marca, $modelo, $descripcion)
{
  try {
...
<strong>' . $nombre_producto . ' Marca: ' . $marca . ' Modelo: ' . $modelo . '</strong><br>
<strong>Descripcion:</strong> ' . $descripcion . '<br>
</p>';
$html .= '<p><strong>Cantidad Entregada:</strong> ' . $cantidad_entregada . '</p>';

$html .= '<p>Este equipo estará asignado al empleado <strong>' . $responsable . '</strong>, 
' . $nombre_departamento . ', comprometiéndose a su uso estrictamente laboral.</p>';

$html .= '<p><strong>Nota de Responsabilidad:</strong> El responsable <strong>' . $responsable . '</strong> se hace cargo de los productos entregados.</p>';

// Sección de firmas
$html .= '<br>';
$html .= '<table width="100%">
    <tr>
        <td width="50%" style="text-align: left;"><strong>RECIBE:</strong><br><br><br>' . $responsable . '</td>
        <td width="50%" style="text-align: left;"><strong>ENTREGA:</strong><br><br><br>'.  $encargado . '</td>
    </tr>
  </table>';
